...
export default function UserScreen() {
  ...
  function redColored(value) {
    return (
      <Text style={{ ...styles.text,fontFamily: fonts.Regular,color: Colors.red}}>{value}</Text>
    );
  }
  ...
  const searchResults = () => {
    ...
    let filter = jsonData.slice(0,searchItemIndex > 9 ? 9 : 10).map((val,ind)=>[searchItemIndex == ind ? redColored(val?.name) : val?.name, ind + 1, val?.bananas, searchItemIndex == ind ? 'yes' : 'no']);
    if(searchItemIndex > 9){
      filter.push([redColored(jsonData[searchItemIndex].name), searchItemIndex + 1, jsonData[searchItemIndex].bananas, 'yes']);
    }
      ...
      <ScrollView style={{marginVertical: Metrix.VerticalSize(20)}}>
        ...
        <View style={styles.searchContainer}>
          ...
          <View style={{width: '28%', marginTop: Metrix.VerticalSize(8)}}>
            <Button buttonText={'Search'} shadow onPress={searchResults} />
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  text: {
    marginHorizontal: Metrix.HorizontalSize(14),
